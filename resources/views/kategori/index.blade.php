...
@section('content')
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content" style="padding: 40px">
                ...
                <div class="d-flex justify-content-center gap-5 mt-4">
                    <button type="button" class="btn w-100" style="width: 25%;border-radius: 20px;border: 1px solid #F6B805; color:#F6B805;" data-bs-dismiss="confirmationModal">Batal</button>
    ...
    <div class="container mb-5">
        ...
        <h4 style="margin-top:70px;color: #1e1e1e;font-size: 28px;font-weight: 400;">Data Kategori</h4>

        <div class="d-flex flex-column flex-md-row justify-content-between">
            <div class="mb-3 mb-md-0" style="width: 100%; max-width: 240px;">
                <a href="/admin/data/kategori/create" class="d-flex align-items-center w-100 btn mt-4 w-100" style="height: 50px;border-radius: 20px;background: #FFF;box-shadow: 0px 0px 8px 0px rgba(0, 0, 0, 0.25);">
            ...
            <div style="width: 100%; max-width: 300px;">
                <div class="mt-md-4 d-flex align-items-center justify-content-start p-3" style="width: 100%;height: 50px;border-radius: 20px;background: #FFF;box-shadow: 0px 0px 8px 0px rgba(0, 0, 0, 0.25);">
                    <span class="w-100 d-flex gap-3" style="font-size: 18px;font-weight: 400;">
                        ...
                        <input class="w-100" type="text" placeholder="Search Kategori..." style="outline:none;border:none;font-size: 18px;font-weight: 400;">
                    </span>
                </div>
            </div>
        </div>
    ...
    <script>
        function loadPage(page, searchQuery = '') {
            ...
            searchTimer = setTimeout(function () {
                $.ajax({
                    url: '/api/categories?page=' + page + '&search=' + searchQuery,
                    headers: {
                    ...
                    success: function ({ data, current_page, per_page, total, last_page }) {
                        ...
                        if (data.length === 0) {
                            row = `
                            <tr>
                                <td colspan="4" style="text-align: center; color: #1e1e1e; font-size: 18px; font-weight: 400;">
                                    No categories found.
                                </td>
                            </tr>`;
                        } else {
                            data.map(function (val, index) {
                                row += `<tr style="border-bottom: 1px solid rgba(30, 30, 30, 0.25);">
                                            <td style="color: #1e1e1e;font-size: 18px;font-weight: 400;">${(page - 1) * per_page + index + 1}</td>
                                            <td style="color: #1e1e1e;font-size: 18px;font-weight: 400;">${val.nama_kategori}</td>
                                            <td style="color: #1e1e1e;font-size: 18px;font-weight: 400;">${val.deskripsi}</td>
                                            <td class="d-flex justify-content-center">
                                                <a href="/admin/data/kategori/edit/${val.id}" class="btn fs-4">
